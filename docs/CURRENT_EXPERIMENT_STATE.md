...
2×2配对评测已完成（见第7节），归因结论：数据来源效应方向性成立、规模无独立贡献。
归因技术报告已沉淀：`docs/M6_PHASE10D_SCALE_DATASOURCE_ATTRIBUTION_REPORT.md`
（证据链A-E、四格全图、归因结论、统计警示、路线建议、产物SHA索引、未决问题）。
**下一步唯一动作（待用户确认）**：按报告第5节与低成本原则，建议以4B为学生推进主链路
`Raw4 < SFT4(D4) < corrected/OPD < RL`——4B专项自纠错/OPD是树上该分支的既定动作；35B规模追问
停止（负结果定案+归因闭环）。若用户另有指向再调整。确认前不提交任何训练/评测作业。
