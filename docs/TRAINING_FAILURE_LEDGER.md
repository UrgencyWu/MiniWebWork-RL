...
| Phase10-C Specialist corpus smoke r1 | 16-task `S_nav_sft`公开query数据构造 | 训练前以`verified-task < 12/16`退出；0 GPU、0 optimizer step、无adapter | 数据门确实未过，但首版脚本在抛错前没有落盘verified计数和拒绝类别，直接原因尚不能区分为公开query召回不足或其他任务不可解 | 保留这次失败记录；最小修复为无论通过/失败都写manifest，再以完全相同16任务复现一次，不改变query、任务或门槛 | 数据构造/可观测性失败，不作为教师能力结论 |
