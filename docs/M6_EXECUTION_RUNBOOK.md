...
mini-SFT 请求 1 GPU、8 CPU、32 GiB。启动时按候选 microbatch `1/2/5` 对最长序列各测 10 个
...
`run_m6_mini_rl_loop_job.sh` 请求 1 GPU、8 CPU、32 GiB，并在一张卡上顺序执行 vLLM K8 采样
和 HF learner，避免 collection 与 learner 同时占两张卡。它还要求 passing `mini/sft_gate.json`。
...
- learner 使用 strict GRPO macro + lambda=0.5 verifier-TD 零和 turn redistribution；
...
| Mini-SFT | 1 | 8 | STOP A passed | 0.5–2 h |
| Mini-SFT K4 eval | 1 | 4 | mini-SFT | 1–2 h |
| Mini-RL loop | 1 | 8 | STOP B1 passed | 0.5–3 h |
