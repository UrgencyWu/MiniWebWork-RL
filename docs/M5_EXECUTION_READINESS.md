...
> 最后更新：2026-08-12
>
> 当前状态：正式在线训练、冻结测试和最终统计分析均已完成；共享服务已停止
>
...
任务为聚类单位；三种子方法区间同时重采样配对训练种子和共同任务；
`Anchor-GiGPO - GRPO` 是最终报告指定的主比较，算法级检验使用配对训练种子的精确双侧符号置换。
由于只有 3 对训练种子，算法级双侧 p 值的理论下限是 0.25；跨 seed 聚合后的配对任务检验仅作为
这些冻结检查点条件下的诊断，不外推为算法总体显著性。
精确分析实现是在冻结评测结束后固化，因此显著性检验应解释为探索性证据，而不是预注册验证。
