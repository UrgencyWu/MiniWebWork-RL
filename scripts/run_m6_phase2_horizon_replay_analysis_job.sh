#!/usr/bin/env bash
# CPU audit for the exact-prefix replay horizon diagnostic.
#SBATCH --job-name=m6-p2-horizon-replay-audit
#SBATCH --partition=compute
#SBATCH --time=01:00:00
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --mem=8G
#SBATCH --output=logs/m6_phase2_horizon_replay_audit_%j.out
#SBATCH --error=logs/m6_phase2_horizon_replay_audit_%j.err
...
"$python_bin" scripts/m6_phase2_horizon_analysis.py \
  ...
  --full-root "$root/full_18_15_prefix_replay_r2" \
  --output "$root/analysis_report_r2.json"
