#!/usr/bin/env bash
# Minimal on-policy Phase4 loop: 4 K4 tasks -> one update, repeated 10 times.
#SBATCH --job-name=m6-p4-online-rl
...
#SBATCH --mem=24G
#SBATCH --gres=gpu:1
#SBATCH --output=logs/m6_phase4_online_rl_%j.out
#SBATCH --error=logs/m6_phase4_online_rl_%j.err
...
: "${M6_PHASE4_ROSTER:?set M6_PHASE4_ROSTER}"
: "${M6_PHASE4_OUTPUT:?set M6_PHASE4_OUTPUT}"
...
steps="${M6_ONLINE_STEPS:-10}"
test "$steps" = 10
test -d "$sft_adapter"
test -f "$M6_PHASE4_ROSTER"
test -f "$split_lock"
test -f "$goals"
...
"$python_bin" - "$M6_PHASE4_ROSTER" <<'PY'
import json,sys
value=json.load(open(sys.argv[1]))
if value.get("task_count") != 40 or len(value.get("task_ids", [])) != 40:
    raise ValueError("Phase4 online roster must contain exactly 40 tasks")
PY

mkdir -p "$M6_PHASE4_OUTPUT"
current_adapter="$sft_adapter"
current_optimizer=""
step=0
while test "$step" -lt "$steps"; do
  step_root="$M6_PHASE4_OUTPUT/step_$(printf '%02d' "$step")"
  ...
  if ! test -f "$collection_root/collection_report.json"; then
    "$slurm_bin/srun" --ntasks=1 "$python_bin" scripts/m6_collect_policy_success.py \
      ...
      --task-roster "$M6_PHASE4_ROSTER" --task-offset "$((step * 4))" --maximum-tasks 4 \
  ...
  if ! test -f "$learner_root/learner_report.json"; then
    "$slurm_bin/srun" --ntasks=1 "$python_bin" scripts/m6_phase4_online_update.py \
      ...
      --microbatch-size 4 "${optimizer_args[@]}"
  fi
...
"$python_bin" - "$M6_PHASE4_OUTPUT" "$current_adapter" "$current_optimizer" "$run_seed" "$sft_adapter" <<'PY'
import json,sys
from pathlib import Path
root=Path(sys.argv[1])
reports=[json.load(open(root/f"step_{i:02d}"/"learner"/"learner_report.json")) for i in range(10)]
value={
    "schema_version":"m6_phase4_online_run_v1",
    "complete":True,
    ...
    "checkpoints":{"step_0":sys.argv[5],"step_5":reports[4]["output_adapter"],"step_10":reports[9]["output_adapter"]},
}
