...
from miniwebwork.webshop_rl.verifier_td import assign_group_credit  # noqa: E402
...
def main() -> None:
    ...
    reports = [validate_learner_report(path) for path in args.iteration]
    ...
    for report, group_dir in zip(reports, args.groups_dir):
        ...
        group_credit = [assign_group_credit(group["trajectories"]) for group in groups]
    ...
    report = audit_mini_rl(learner_report=learner, credit_assignments=credit)
