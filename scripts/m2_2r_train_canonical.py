"""M2.2R Canonical Training: LoRA SFT on canonical dataset (m2_2r).

Uses same training code as M2.2 but with canonical prompt contract data.
"""
import json
import os
import sys
...
def tokenize_with_completion_mask(example, tokenizer, max_length):
    """Tokenize with completion-only labels (same as train_m2_2.py)."""
...
def prepare_dataset(ds, tokenizer, max_length, num_proc=1):
    def tokenize_fn(example):
        return tokenize_with_completion_mask(example, tokenizer, max_length)
    tokenized = ds.map(tokenize_fn, remove_columns=ds.column_names, num_proc=num_proc, desc="Tokenizing")
    print(f"  Tokenized: {len(tokenized)} samples")
    return tokenized


def compute_action_metrics(model, tokenizer, raw_dataset, max_length):
    """Compute teacher-forced action accuracy (same as train_m2_2.py)."""
    model.eval()
    exact_match = 0
    action_type_match = 0
    schema_valid = 0
...
def train_single_seed(base_model_path, train_dataset, eval_dataset, output_dir, seed,
...
def main():
    ...
    print("=== M2.2R Canonical SFT Trainer ===")
    ...
    # Load datasets
    print("\nLoading datasets...")
    train_ds = _hf_load_dataset("json", data_files={"train": str(args.data_dir / "train.jsonl")}, split="train")
    valid_ds = _hf_load_dataset("json", data_files={"valid": str(args.data_dir / "valid.jsonl")}, split="valid")
    ...
    for seed in args.seeds:
        ...
        final_adapter = seed_dir / "final_adapter"

        # Skip if already completed
        if final_adapter.exists() and (final_adapter / "adapter_config.json").exists():
            print(f"\n{'='*60}")
            print(f"Seed {seed}: ALREADY COMPLETED (adapter exists)")
            print(f"{'='*60}")
        ...
        # Find latest checkpoint for this seed
        checkpoint = None
        if seed_dir.exists():
            checkpoints = sorted(seed_dir.glob("checkpoint-*"))
            if checkpoints:
                checkpoint = str(checkpoints[-1])
                print(f"\nSeed {seed}: Resuming from {checkpoint}")
    ...
    summary_path = args.output_dir / "training_summary_canonical.json"
