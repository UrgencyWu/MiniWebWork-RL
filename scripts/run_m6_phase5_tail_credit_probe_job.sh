#!/usr/bin/env bash
#SBATCH --job-name=m6-p5-tail-credit
...
#SBATCH --mem=24G
#SBATCH --gres=gpu:1
#SBATCH --output=logs/m6_phase5_tail_credit_%j.out
#SBATCH --error=logs/m6_phase5_tail_credit_%j.err
...
output="${M6_PHASE5_OUTPUT:-$study/phase5_tail_credit_v1/report.json}"
test ! -e "$output"
export PYTHONPATH="$repo_root/src${PYTHONPATH:+:$PYTHONPATH}"
export TOKENIZERS_PARALLELISM=false
"$python_bin" scripts/m6_phase5_tail_credit_probe.py \
